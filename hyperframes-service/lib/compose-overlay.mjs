 ...
 *  - mode "distinct": input 1..N = N đoạn C2 (mỗi đoạn 1 cảnh khác); caption (nếu có) = input N+1.
 *  - mode "loop"    : input 1 = C2 (-stream_loop) [fallback khi c2Dur lỗi]; caption = input 2.
 *  - mode "none"    : không có cutaway; caption (nếu có) = input 1.
 * @returns {{ mode:"distinct"|"loop"|"none", c2Offsets:({offset:number,readDur:number}[]|null), filter:string, captionIdx:(number|null) }}
 */
export function buildComposeGraph({ scaleCrop, segs, c2Dur, hasCaption }) {
  ...
  if (distinct) {
    ...
    list.forEach((s, k) => {
      const out = `o${k}`;
      chain.push(`[${cur}][seg${k}]overlay=enable='between(t,${Number(s.start).toFixed(2)},${Number(s.end).toFixed(2)})':eof_action=pass[${out}]`);
      cur = out;
    });
  } else if (list.length > 0) {
    mode = "loop";
    nextInput++; // input 1 = C2 (-stream_loop)
    const expr = list.map((s) => `between(t,${Number(s.start).toFixed(2)},${Number(s.end).toFixed(2)})`).join("+");
