...
- 🔥 **C3 Round 2 — làm giàu DATA + chiều sâu** (ĐÃ push `916352d` deploy READY · scripter VERIFY LIVE): **point scenes = THẺ HƯỚNG DẪN giàu data** (tiêu đề bước + cách làm/DẠY + thẻ số liệu, **2 thẻ xếp lớp + watermark số → chiều sâu**), **mini-stat hết hở giữa** (căn giữa), **CTA mũi tên**, **wiring scripter+builder** sinh `animation.points:[{title,detail,stat}]` (fallback keyMessages, `text=title` backward-compat). ✅ Gen script thật live (`f9ed585c`): scripter ra ĐÚNG points giàu + **grounded 8 nguồn thật**; render LOCAL data thật đẹp; edge-test 6 ca PASS; tsc+build PASS. ⏳ **VIỆC TIẾP DUY NHẤT: Tommy scp composition VPS + rebuild → verify point cards giàu trên VIDEO thật** (chưa scp thì point hiện title-only, KHÔNG vỡ). Chi tiết: `BLUEPRINT_C3_V2.md` mục 8.
