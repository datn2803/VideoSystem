...
- 🔥 **C3 Round 2 — làm giàu DATA + chiều sâu** (đợt mới, code XONG, render-test LOCAL dark+bright PASS, tsc+build PASS — **CHƯA push/scp**): **point scenes = THẺ HƯỚNG DẪN giàu data** (tiêu đề bước + cách làm/DẠY + thẻ số liệu, **2 thẻ xếp lớp + watermark số → chiều sâu**), **mini-stat hết hở giữa** (căn giữa), **CTA mũi tên** (bỏ emoji tay), **wiring scripter+builder** sinh `animation.points:[{title,detail,stat}]` (fallback keyMessages). Edge-test 6 ca PASS. ⏳ **VIỆC TIẾP: push main (Vercel) + Tommy scp composition VPS → verify video tài chính**. Chi tiết: `BLUEPRINT_C3_V2.md` mục 8.
