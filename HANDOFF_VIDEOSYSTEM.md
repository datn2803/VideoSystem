...
- 🏗 **ĐẠI TU RENDER & THIẾT KẾ (Jun 2026, máy Windows, kế hoạch `KE_HOACH_NANG_CAP_RENDER.md` ở repo chính C:\Users\Admin\VideoSystem)** — làm tuần tự 6 phase trên worktree `modest-mahavira-0b3db5`:
  ...
  - 🔴 **CHECKPOINT CHỜ TOMMY** (chưa làm được từ máy này): (1) **push** `git push origin HEAD:main` (Vercel deploy — 11 commit); (2) **scp VPS + rebuild** (animation.html, broll.html, lib/vision.mjs, lib/render-engine.mjs, server.mjs, package.json, Dockerfile — lệnh trong tổng kết); (3) verify production + render thật (RENDER_MODE=live khi muốn C1/C2 ảnh AI); (4) revoke 3 key lộ (SECURITY.md); (5) muốn nhạc → đặt MINIMAX_API_KEY; (6) muốn lõi playwright → env RENDER_ENGINE=playwright trong docker-compose.
