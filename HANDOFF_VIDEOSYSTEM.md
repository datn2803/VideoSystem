...
- 🔴 **CỰC KỲ QUAN TRỌNG — làm ĐẦU TIÊN:** thư mục chính đang ở branch `Tommy` **TỤT ~16 commit** sau `origin/main` → THIẾU file (vd `src/lib/audio/whisper.ts`) → **`npm run build` sẽ VỠ** nếu code trên trạng thái này. Phải đồng bộ trước:
  ```bash
  cd "<repo chính>"
  git fetch origin
  git reset --hard origin/main      # đưa working copy = main mới nhất (bỏ mọi thay đổi rác cũ)
  npm install                        # đảm bảo deps khớp
...
→ gồm `MEMORY.md`, `hyperframes-render-service.md` (kiến trúc render C2/C3 + C3 v5 read-script model + levers tốc độ), `leaked-keys-to-revoke.md`. (Cùng máy → mọi tài khoản Claude Code đọc được; nếu không thấy thì HANDOFF này đã đủ context.)
