...
- 🏗 **ĐẠI TU RENDER & THIẾT KẾ (Jun 2026, máy Windows, kế hoạch `KE_HOACH_NANG_CAP_RENDER.md` ở repo chính C:\Users\Admin\VideoSystem)** — làm tuần tự 6 phase trên worktree `modest-mahavira-0b3db5`:
  ...
  - ✅ **Phase 2** (commit `3337dcd`+`e530471`): Design Director deterministic $0 (`src/lib/design/`: token derive từ anchor hex catalog qua HSL — bộ OKLch→hex có sẵn đã test, để dành cho directions; director chọn hệ theo industry→deriveTokens, BrandKit KV keyed profileId = ĐA BRAND) + UI BrandKitPanel (đổi hệ/skin/accent) + builder C2/C3 truyền `tokens`+`visionQC` + **composition token hoá** (animation: 19 tint tím→var --acc-aXX theo accent thật + skin classic/poster/editorial; broll: accent/grade + skin) + vision.mjs QC 5 chiều/blacklist (giữ contract score 0-10) + manifest yaml ×2. Verify LOCAL 4 kit × 4 cảnh: token ăn đúng, không tràn, default = look cũ nguyên vẹn; broll smoke 0 lỗi. Test scripts: `_p2_render_test.mts`/`_p2_broll_smoke.mts` (gitignored), screenshots `C:\Users\Admin\VideoSystem\_p2_shots\`. ⚠ CHƯA scp VPS (animation.html + broll.html + lib/vision.mjs) — checkpoint.
  - ✅ **Phase 3** (`d8311fc`+`8bcbc37`): `src/lib/video/engine.ts` EngineAdapter (render/poll) bọc cả RenderProvider + avatar; `cost-guard.ts` RENDER_MODE=mock|dryrun|live (default dryrun, compat RENDER_LIVE=1) + trần DAILY_COST_CAP_USD ($5) assertDailyCap trước MỌI call PAID + usage meter; cache renderHash mở rộng C2/C3 (content+audio+BrandKit; force bỏ cache); buildAll song song p-limit (RENDER_BUILD_CONCURRENCY=2); UI confirm-before-spend khi live. VPS: `lib/render-engine.mjs` lõi Playwright+ffmpeg FRAME-STEPPING (tl.time từng frame→screenshot→pipe libx264) sau cờ `RENDER_ENGINE=playwright` (default cli như cũ) + semaphore `RENDER_CONCURRENCY` (default 1) + Dockerfile cài playwright chromium. **Verify local: render MP4 thật 6s/144frame/15.9s wall — frame đẹp, BrandKit ăn trọn** (`C:\Users\Admin\VideoSystem\_p3_out\`).
  - ✅ **Phase 4** (`d6c293c`): SceneStudio trên trang script (sửa chữ/số/thời lượng/đổi thứ tự từng cảnh storyboard + preview iframe srcdoc theo BrandKit + render RIÊNG 1 cảnh qua engine $0 — `scene-preview.ts` map node→biến tối thiểu); vendor `fetch-source` (chặn SSRF) → "Dán link/bài → video" trên /scripts (bài = source brief, số có nguồn, vẫn qua Auditor).
  - ✅ **Phase 5** (`ccf1dd2`): PipelineRunner 1-lệnh trên card profile (Plan→Script→Voice→Render, client tuần tự < cap 60s); nhạc MiniMax tuỳ chọn (`minimax-music.ts` music-1.5 port html-video, key env MINIMAX_API_KEY, gate live+trần+extra-usage; VPS `/audio/mix` duck -18dB + builder tự mix khi có track music — cache mix theo voiceId::musicId); export: hook LỊCH ĐĂNG (scheduledAt + UI) — caption đa nền tảng đã có sẵn.
  - ✅ **Chốt compliance** (`f47cb5a` — lỗ hổng phát hiện lúc tự kiểm): orchestrator chặn MỌI đường render khi audit.status==="fail" (verify e2e: inject script vi phạm → 0 video, message rõ).
  - 🔬 **Đã verify local (mock, $0)**: e2e pipeline thông; studio sửa/lưu/đổi thứ tự persist; link→video; pipeline 1-lệnh; cost-guard 8/8 (mode default dryrun, trần throw); compliance block; render-test 4 BrandKit × 4 cảnh không tràn; lõi playwright ra MP4 thật. tsc+build PASS mọi commit.
  - 🔎 **Subagent review độc lập toàn diff** (1 CAO + 5 TRUNG + 12 THẤP) → ĐÃ VÁ 11 finding ở commit `d8ac709` (CAO: unhandledRejection sập service ở lõi playwright; TRUNG: hash cache thiếu nhạc, compliance hở scene-preview, preview hook/outro nửa trống, SSRF lách redirect/IP-shorthand; THẤP: visionQC sau gate live, whisper trên voice gốc, leak tmpDir QC, theme preview lệch, interval leak, gitignore temp, manifest lệch). NỢ CÒN LẠI (đã ghi nhận, không vá đợt này): **T2 — Scene Studio sửa storyboard CHƯA chảy vào video chính** (graph→variables build là việc kế tiếp; hiện studio = preview/sửa cảnh lẻ, video chính vẫn dựng từ variantPrompts), t3 race brandkits hiếm, t4 TOCTOU trần/ngày khi build song song, t6 shim playwright không merge default biến, t8 /audio/mix limit 16mb + đổi duck không vô hiệu cache mix, t12 trần reset theo UTC.
  - 🛠 **VÁ REVIEW ĐỢT 2** (commit `7c1e263`+`8faf8df`+`437f238`, theo prompt vá của Tommy): **P0 rò tiền** — preview ảnh + TTS ElevenLabs + hero + Whisper nay ĐỀU gate isLive + trần ngày (⚠ **HÀNH VI ĐỔI CÓ CHỦ ĐÍCH: dryrun → TTS mock im lặng; production muốn GIỌNG THẬT phải đặt `RENDER_MODE=live`** — có confirm UI + trần $5/ngày bảo vệ); **P1** — trần ngày chống TOCTOU (serialize + đặt chỗ TTL 5', test 3-song-song đúng 2 qua), SSRF thêm chặn DNS rebinding (resolve từng hop, localtest.me bị chặn), `RENDER_PIPELINE_VERSION` vào renderHash (bump tay khi đổi composition → cache tự vô hiệu), **broll nền theo BrandKit mode** (brand sáng = nền sáng — verify 3 chế độ); **P2** — scene-planner đánh dấu ĐỂ DÀNH chờ quyết graph-driven, badge UX "preview cảnh lẻ" trong Studio, đính chính phát biểu quá lời. NGOẠI LỆ GATE CÓ CHỦ ĐÍCH: LLM Gemini (core flow, free-tier) không gate isLive; VPS $0 chỉ tắt ở mock.
  - 🛠 **VÒNG REVIEW ĐỢT VÁ** (commit `c985647`, subagent độc lập: 0 CAO/4 TRUNG/6 THẤP → vá T1-T4+L1): TTS est default 0.22/1k khớp ElevenLabs (reserve hết yếu 4.4×); hero record sau generate; broll chữ title/stat token theo mode (light = ink tối — verify mắt nền trắng chữ đen); **Creatomate gate isLive+trần** (trả phí, không phải self-host); Whisper thêm assert. Nợ THẤP còn lại ghi nhận: mock TTS bỏ qua speed (duration dryrun dài hơn ~20%), mock-mode vẫn gọi VPS atempo ($0), reserve treo ≤5' hướng over-block, SSRF chưa chặn CGNAT/NAT64 + TOCTOU DNS (cần pin-IP), bump RENDER_PIPELINE_VERSION trước scp → 1 lần render lại bằng composition cũ ($0).
  - 🟠 **CHỜ TOMMY QUYẾT — graph-driven build**: storyboard hiện CHƯA điều khiển video chính (chỉ preview cảnh lẻ + Studio). Muốn Scene Studio "thật" → nối scene-planner vào builder = 1 phase riêng có cờ bật/tắt. Chưa quyết thì giữ preview-only như hiện tại.
  - ✅ **ĐÃ PUSH MAIN** (Tommy duyệt, 2026-06-11): `7edad03..4933e79` — 20 commit lên `main`, Vercel auto-deploy (commit cuối `4933e79` = cổng lint xanh: .eslintrc + vá hooks voice-studio). **Kiểm thử toàn diện trước push**: lint ✔ 0 lỗi (cổng lần đầu chạy) · tsc 0 · build ✓ · 47 test offline (graph 17, costguard 8, ssrf 12, guard 4, parse 6) · composition 4 kit + broll 3 nền + MP4 engine 6s · e2e mock 6 bước + studio + pipeline + compliance + smoke (BrandKit đổi hệ persist, storyboard hợp lệ, console 0 lỗi, 0 request host trả phí, usage $0).
  - 🔴 **CHECKPOINT CÒN LẠI CHỜ TOMMY**: (1) **scp VPS + rebuild** (animation.html, broll.html, lib/vision.mjs, lib/render-engine.mjs, server.mjs, package.json, Dockerfile); (2) verify production + render thật (RENDER_MODE=live khi muốn giọng thật/C1/C2 ảnh AI — ⚠ sau vá P0, dryrun = TTS mock im lặng); (3) revoke 3 key lộ (SECURITY.md); (4) muốn nhạc → MINIMAX_API_KEY; (5) muốn lõi playwright → RENDER_ENGINE=playwright trong docker-compose.
